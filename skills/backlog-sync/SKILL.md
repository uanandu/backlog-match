...
allowed-tools: [mcp__*, Read, Grep, Glob, "Bash(git:*)"]
license: MIT
metadata:
...
6. Gather the link for the comment and sub-task, from the least to most effort, and never invent one:
   1. A PR URL, using only `git`: search history for the merge — `git log --oneline --grep="<KEY>"` and `--grep="<change name>"`, plus merge commits (`git log --merges --oneline`). A subject like `Merge pull request #17 from …` or a squash-merge subject ending in `(#17)` gives the PR number. Build the URL from `git remote get-url origin` (normalize `git@github.com:owner/repo.git` and `https://…/repo.git` to `https://github.com/owner/repo`) plus `/pull/17`. Only do this for a GitHub-style host you recognize.
   2. Otherwise a commit URL: the commit that mentions the key or change name, as `<repo URL>/commit/<full sha>`, again only for a recognized host.
